...
\begin{frame}{Vaccination Problem}
    ...
    \visible<2->{
      \begin{tcolorbox}[title=Herd immunity]
        \textbf{\textit{Herd immuntiy}} occurs when a large enough fraction $f$ of a community is immune to a disease, thus limiting its ability to spread.
      \end{tcolorbox}
    }
    \visible<3->{
...
\begin{frame}{Vaccination Problem on static graphs}
  ...
  \begin{center}
    \begin{tikzpicture}[node distance=1.5cm, every node/.style={draw, circle, minimum size=0.8cm}]
        ...
        \node (I) [right of=F] {I};
        \visible<4, 5>{\node[fill=blue!30] (I) [right of=F] {I};};
...
\begin{frame}{Modelling dissemination processes}
  ...
  \begin{center}
    \begin{tikzpicture}[node distance=2cm, every node/.style={draw, circle, minimum size=0.5cm}]
        ...
        \path (F) edge node[above, draw=none, font=\tiny, text=blue, inner sep=1pt] {3, 4, 5} (I);
   \end{tikzpicture}
  \end{center}
  }
\end{frame}
