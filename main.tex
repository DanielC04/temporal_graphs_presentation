...
\begin{frame}{Exercise notation}
  \large
  \italic{Given the following temporal graph definition $D = (V, E, \lambda)$ draw the visual representation of the temporal graph like in the previous example!} \hfill \tiny 2 min \\
  \normal
  $$ V = \{ C, H, B, P, M, G, W \} $$
  $$ E = \{ (C, H), (H, B), (B, G), (G, W), (W, P), (H, P), (B, M), (M, G) \} $$
  $ \lambda: E \rightarrow 2^{\mathbb{N}} = \{ \newline
    \hspace*{1cm}  (C, H) \mapsto \{ 4, 6 \}, \newline
    \hspace*{1cm}  (H, B) \mapsto \{ 4, 6 \}, \newline
    \hspace*{1cm}  (B, G) \mapsto \{ 4, 6 \}, \newline
    \hspace*{1cm}  (G, W) \mapsto \{ 4, 6 \}, \newline
    \hspace*{1cm}  (W, P) \mapsto \{ 4, 6 \}, \newline 
    \hspace*{1cm}  (H, P) \mapsto \{ 4, 6 \}, \newline
    \hspace*{1cm}  (B, M) \mapsto \{ 4, 6 \}, \newline
    \hspace*{1cm}  (M, G) \mapsto \{ 4, 6 \} \newline
        \} $
...
\begin{frame}{Solution}
  ...
  % Define edges with timestamps
  \path[->] (C) edge node[above left] {\{4, 6\}} (H);
  \path[->] (H) edge node[above right] {\{4, 6\}} (B);
  \path[->] (B) edge node[above left] {\{4, 6\}} (G);
  \path[->] (G) edge node[above right] {\{4, 6\}} (W);
  \path[->] (W) edge node[below] {\{4, 6\}} (P);
  \path[->] (H) edge node[left] {\{4, 6\}} (P);
  \path[->] (B) edge node[below] {\{4, 6\}} (M);
  \path[->] (M) edge node[below right] {\{4, 6\}} (G);
